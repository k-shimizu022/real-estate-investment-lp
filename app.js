document.addEventListener('DOMContentLoaded', function() {
    ...
    // 固定CTAボタンの要素を取得
    const fixedCtaButton = document.getElementById('fixed-cta-btn');

    // スクロール時にボタンの表示/非表示を切り替える関数
    function toggleFixedCtaButton() {
        if (!fixedCtaButton) return; // ボタン要素が見つからない場合は何もしない
        ...
        // 画面の高さの半分以上スクロールされたら表示
        if (scrollPosition > windowHeight / 2) {
            fixedCtaButton.classList.add('visible'); // visibleクラスを追加して表示
            fixedCtaButton.classList.remove('hidden'); // hiddenクラスは削除（もし残っていれば）
        } else {
            fixedCtaButton.classList.remove('visible'); // visibleクラスを削除して非表示
        }
    }

    // スクロールイベントに関数を紐付け
    window.addEventListener('scroll', toggleFixedCtaButton);

    // 初期表示時にも一度チェック（リロード時に既にスクロールされている場合のため）
    toggleFixedCtaButton();
});